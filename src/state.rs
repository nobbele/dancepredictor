...
pub struct State {
    ...
    pub hitting_columns: FootPlacement,
...
impl State {
    pub fn new(column_count: usize) -> State {
        State {
            ...
            hitting_columns: FootPlacement::new(column_count),
    ...
    pub fn append(&self, columns: &FootPlacement) -> State {
        ...
        assert_eq!(column_count, self.hitting_columns.0.len());
        assert_eq!(column_count, self.final_columns.0.len());
        ...
        for column in 0..column_count {
            ...
            if column_value != FootPart::None {
                *result.hitting_columns.at_mut(column) = column_value;
        ...
        for column in 0..column_count {
            if result.hitting_columns.at(column) != FootPart::None {
                *result.final_columns.at_mut(column) = result.hitting_columns.at(column);
                continue;
            };

            let prev_foot_part = self.final_columns.at(column);
            if prev_foot_part != FootPart::None && !result.hitting_columns.contains(prev_foot_part)
            {
    ...
    pub fn foot_part_activated(&self, part: FootPart) -> bool {
        self.hitting_columns.contains(part)
    }
...
mod tests {
    ...
    fn test_state_progression() {
        ...
        let state = state.append(&FootPlacement(vec![
            FootPart::LeftHeel,
            FootPart::None,
            FootPart::None,
            FootPart::None,
